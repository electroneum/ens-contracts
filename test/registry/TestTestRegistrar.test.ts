...
describe('TestRegistrar', () => {
  it('registers names', async () => {
    ...
    await testRegistrar.write.register([labelhash('etn'), accounts[0].address])
  ...
  it('forbids transferring names within the test period', async () => {
    ...
    await testRegistrar.write.register([labelhash('etn'), accounts[1].address])

    await expect(
      testRegistrar.write.register([labelhash('etn'), accounts[0].address]),
