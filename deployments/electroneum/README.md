...
- Multicall3 cannot deploy at its canonical cross-chain address (`0xcA11…CA11`) on Electroneum: that deployment is a pre-signed, pre-EIP-155 transaction, which etn-sc rejects. The pipeline detects this and deploys Multicall3 normally at a **non-canonical address** (recorded in the deployment files); `20_set_tlds` uses the recorded address automatically. If you configure viem/wagmi clients with a custom Electroneum chain definition, point their `multicall3` contract entry at the recorded address.
