...
export default deployScript(
  async ({
    ...
    if (resolver === zeroAddress) {
      console.warn(
        `  - WARN: No resolver set for .etn; not setting interface ${interfaceId} for ETHRegistrarController`,
      )
      return
    }

    console.log(
      `  - Setting ETHRegistrarController interface ID ${interfaceId} on .etn resolver`,
    )
    await write(
      { ...artifacts.OwnedResolver, address: resolver },
      {
        functionName: 'setInterface',
        args: [namehash('etn'), interfaceId, controller.address],
        account: owner,
      },
    )
